...
def parse_config(config_file):
    ...
    try:
        ...
        scaling = float(config['COMMON']['scaling'])
        ...
        if sequenced_data_path:
            logging.info("Sequencing data and a mean read length were provided...")
            logging.info("Only sequencing data will be used for the read length distribution...")
            mrl = int(mean_read_length(sequenced_data_path))
            logging.info(f"Mean read length is set to your data specific mean: {mrl}")
        ...
        if mode == "ROI":
            ...
            logging.error("No valid running mode selected.")
            sys.exit()
...
def parse_config(config_file):
    ...
    try:
        ...
        min_overlap_for_detection = config.getint("COMMON", "min_overlap_for_detection", fallback=100)
