<p align="center">
    <img src="esloco_logo.png" alt="Illustration" width="200">
</p>
...
1. **Genome Preparation**: Barcoded genomes are generated by converting the reference genome into a single coordinate space. Random insertions (`[I]` mode) or user-defined regions of interest (`[ROI]` mode) are assigned as target regions.

2. **Read Simulation**: For each barcode, reads are drawn from a user-defined or empirical read length distribution and randomly placed until the desired whole-genome coverage is reached. Reads falling into blocked regions are excluded. Local coverage for each target is calculated based on on-target bases of overlapping reads. 
...
For a more comprehensive analysis, `plot_esloco {config.ini}` can generate additional overview plots. These are summarized in an interactive `{experiment_name}_report.html`. Below is an exemplary overview for the figures encountered in the report.
